...
export function ChatButtonEditable({ colorPalette, onEvent, value, ...props }: Omit<ButtonProps<[TextEditEvent]>, "children">) {
  return (
    <AutoLayout
      name="ChatButton"
      effect={[
        { type: "drop-shadow", color: "#0000001F", offset: { x: 0, y: 4 }, blur: 10.4, showShadowBehindNode: false },
        { blur: 16.8, type: "background-blur" },
      ]}
      cornerRadius={6}
      width={"fill-parent"}
      height={40}
...
export function Button({ colorPalette, children, onEvent, ...props }: ButtonProps) {
  ...
  return (
    <AutoLayout
      name="Button"
      onClick={onEvent}
      hoverStyle={{ fill: colorPalette.surface?.primaryHover }}
      effect={[
        { type: "drop-shadow", color: colorPalette.surface?.inputBg as string, offset: { x: 0, y: 2 }, blur: 3 },
        { type: "drop-shadow", color: "#00000026", offset: { x: 0, y: 6 }, blur: 10, spread: 4 },
      ]}
      fill={colorPalette.surface?.primary}
      cornerRadius={8}
      spacing={12}
      padding={{ top: 16, right: 64, bottom: 16, left: 32 }}
      width="fill-parent"
      horizontalAlignItems="center"
      verticalAlignItems="center"
      {...props}
